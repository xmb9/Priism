...
# Priism 2.0 - shmenu.sh
# Based on https://github.com/duclos-cavalcanti/shmenu
        ...
        while [[ $# -gt 0 ]]; do
           case $1 in
                -o|--options)
                    ...
                    # features
                    IFS=$'\n' read -r -d '' -a _OPTIONS < <(printf "%s\0" "$1")
                    shift
                    ;;
...
main() {
	...
	read -rsn1 _INPUT # get 1 character
	if [[ $_INPUT == $escape_char ]]; then
    		read -rsn2 _INPUT # read 2 more chars
	fi
        case ${_INPUT} in
            '[B') # down
...
main "$@"
[ -n "${_CHOSEN}" ] && printf "${_CHOSEN}" > /tmp/shmenu_choice || exit 0
